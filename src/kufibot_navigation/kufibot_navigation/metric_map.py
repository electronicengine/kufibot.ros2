...
class MetricMap:
    ...
    def ray(self, origin, bearing_deg, distance, hit=True, measured_at=None):
        """Accumulate static geometry; a later free ray never erases a wall.

        Conflicting rays stop adding free cells at the first known obstacle.
        ...
        changed = False
        for key in cells:
            if key in self.occupied or (hit and key == end):
                break
        ...
        if hit:
            ...
            if end in self.occupied:
                pass  # Do not move or evict a previously measured wall.
    ...
    def snapshot(self, pose, heading, source):
        ...
        if self._boundary_signature != signature:
            self._boundaries = boundary_paths(list(self.occupied.values()))
        ...
        return dict(map_id=self.id, revision=self.revision, frame='startup_robot_pose', units='m',
                    ...
                    wall_policy='persistent_until_map_reset', occupied_capacity_reached=self.capacity_reached,
