...
def test_websocket_commands_video_and_disconnect():
    async def scenario():
        control = Control()
        server = Server(control, lambda: {'mode': control.mode}, lambda: 'jpeg-test')
        async with TestClient(TestServer(server.app)) as client:
            ...
            video = await client.ws_connect('/video')
            assert (await video.receive()).data == 'jpeg-test'
            await video.close()
            await ws.close()
            for _ in range(10):
